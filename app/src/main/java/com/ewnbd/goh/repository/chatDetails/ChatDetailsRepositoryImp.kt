package com.ewnbd.goh.repository.chatDetails

import android.util.Log
import com.ewnbd.goh.data.model.response_all_class.chatDetails.ChatDetailsResponse
import com.ewnbd.goh.data.remote.ApiInterface
import com.ewnbd.goh.utils.Resource
import retrofit2.HttpException
import java.io.IOException
import javax.inject.Inject

class ChatDetailsRepositoryImp @Inject constructor(
    private val api: ApiInterface
): ChatDetailsRespository{
    override suspend fun chatRepository(
        header: Map<String, String>,
        chatId: String
    ): Resource<ChatDetailsResponse> {
        val response = try {
            api.chatDetails(header,chatId)
        }catch (ioExeption: IOException){
            Log.e("CheckCall", "loginResponse: $ioExeption" )
            return Resource.Error(ioExeption.toString())

        }
        catch (e: HttpException){
            Log.e("CheckCall", "loginResponse hlw: "+e.response().toString()+"  asdad "+e.code())
            return Resource.Error(""+e.code())
        }
        Log.e("CheckCall", "loginResponse: ")
        return Resource.Success(response)
    }

}